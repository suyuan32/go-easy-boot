...
func (l *LoginLogic) Login(req *types.LoginReq) (resp *types.LoginResp, err error) {
	if ok := store.Verify(req.CaptchaId, req.Captcha, true); ok {
		...
		if user.Code == uint32(codes.OK) {
			token, err := l.getJwtToken(l.svcCtx.Config.Auth.AccessSecret, time.Now().Unix(),
				l.svcCtx.Config.Auth.AccessExpire, int64(user.Id), int64(user.RoleId))
			if err != nil {
				return nil, err
			}
...
func (l *LoginLogic) getJwtToken(secretKey string, iat, seconds, userId, roleId int64) (string, error) {
